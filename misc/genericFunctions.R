...
#' Cria Conexao com BD MySQL OpenMRS
#'
#' @return objeto de conexao com BD OpenMRS
#' @examples con_openmrs <- getOpenMRSCon("dbname")
getOpenMRSCon <- function(db.name) {
    ...
    rs  <-
      dbSendQuery(
        ...
        paste0(
          ...
          pe.uuid,
          pe.birthdate,
          concat(pn.given_name,if(pn.middle_name is not null,concat(' ', pn.middle_name)  ,''),concat(' ',pn.family_name) ) as nome_completo ,
          estado.estado as estado_tarv ,
          DATE_FORMAT(estado.start_date,'%d/%m/%Y' ) data_estado,
          DATE_FORMAT(visita.encounter_datetime,'%d/%m/%Y') as data_ult_consulta,
         DATE_FORMAT(visita.value_datetime,'%d/%m/%Y') as data_prox_marcado,
          ...
          LEFT JOIN
        		...
        		) estado ON estado.patient_id=pat.patient_id
         LEFT Join
             (
        		...
        		) visita  on visita.patient_id=pat.patient_id
		...
		(SELECT ultimavisita.patient_id,ultimavisita.encounter_datetime,o.value_datetime,e.location_id
			...
			e.encounter_type=18
		) ultimoFila ON ultimoFila.patient_id=pat.patient_id
